  ...
  try {
    ...
    const r = await fetch(`${VERCEL_URL}/api/ebay`, {
      ...
      body: JSON.stringify({
        ...
        fallbackTitle:   product.ebayTitle || product.title || '',
        fallbackPrice:   product.cost || product.amazonPrice || product.myPrice || 0,
        fallbackShipping: product.shippingCost || 0,
        // Pass full combo data — critical for variation products
        // Never rely on product-level inStock for variations (stale data causes all-OOS)
        fallbackInStock:  product.hasVariations ? true : ((product.quantity||1) > 0),
